...
public class EnemyHide : MonoBehaviour
{
    ...
    public void Update (){
        if (fieldOfView.canSeePlayer == true){
            HandleGainSight (fieldOfView.playerRef.transform);
            animController.Play("Run");
        }else{
            animController.Play("Crouching Idle");
        }
    }
    ...
    private IEnumerator Hide(Transform Target)
    {
        ...
        while (true)
        {
            ...
            int hits = Physics.OverlapSphereNonAlloc(agent.transform.position, fieldOfView.radius, Colliders, fieldOfView.obstructionMask);
